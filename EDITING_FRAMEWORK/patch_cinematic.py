...
f = Path("cinematic.html")
html = f.read_text()

# 1. Update title
html = html.replace("<title>Josh Memorial — Player</title>", "<title>Josh Memorial — Cinematic Player</title>")
...
js_new = """
  ...
  if (usePush) {
      ...
      currentGsapTl.fromTo(bgLayer, 
          { scale: 1.0, x: -20 },
          { scale: 1.05, x: 20, duration: dur + transDur, ease: "none" }, 
          0
      );
      
      // Foreground faster push/drift
      currentGsapTl.fromTo(fgLayer,
          { scale: (scale/100), x: -40 },
          { scale: (scale/100) + 0.08, x: 40, duration: dur + transDur, ease: "power1.out" },
          0
      );
  }
